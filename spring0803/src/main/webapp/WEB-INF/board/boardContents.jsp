...
$.boardCommentList = function(){
	...
	$.ajax({
		...
		url : "<%=request.getContextPath()%>/comment/<%=bv.getBidx()%>/commentList.do",	// commentList를 가져오게끔 가상경로를 만들어줌, 상태정보를 파라미터로 넘기지 않고 주소사이에 값을 넘기는 방식 -> restfull 방식 
		dataType : "json",
		cache : false,
		success : function(data){
			//alert("통신성공");
...
function commentDel(cidx) {	//commentDel는 매개변수로 cidx를 받겠다
	...
	$.ajax({
		...
		url : "<%=request.getContextPath()%>/comment/commentDelete.do?cidx="+cidx,	//commentDelete를 가져오게끔 가상경로를 만들어줌
		dataType : "json",
		cache : false,
		success : function(data){
			alert("삭제 성공");
